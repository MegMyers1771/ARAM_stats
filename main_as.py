...
def parse_to_txt(t1, t2, kills, w_check, id_):
    ...
    with open ('Stats_AS.txt', 'a+') as stats_file:
        if w_check:
...
def getting_gamelists(reg_, names_collection):
    ...
    for items in puuid_list:
        try:
            response = requests.get(
                f"https://asia.api.riotgames.com/lol/match/v5/matches/by-puuid/{items}/ids?start=0&count=20&api_key={api_k}"
            ).json()
    ...
    for match_ in all_games_list:
        ...
        try:
            ...
            response = requests.get(
                f"https://asia.api.riotgames.com/lol/match/v5/matches/{match_}?api_key={api_k}"
            ).json()
            os.system('cls')
